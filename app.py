...
def decompress_block(data, offset):
    chunk = data[offset:offset+800000]
    for method in [
...
def find_xml(data):
    # Search for zlib compressed blocks
    for i in range(len(data)-6, max(0, len(data)-1000000), -1):
        if i+1 >= len(data):
            continue
        b0, b1 = data[i], data[i+1]
        if b0 == 0x78 and b1 in (0x01, 0x9c, 0xda, 0x5e):
            out = decompress_block(data, i)
            if out and (b'QuestionBlock' in out or b'<?xml' in out):
                try:
...
def emf_to_png(emf_bytes):
    """Convert EMF bytes to PNG base64 using LibreOffice"""
    try:
...
def find_emf_images(data):
    """Find all EMF images in binary data"""
    images = []
    i = 0
    while i < len(data) - 100:
        # EMF starts with record type 1 (EMR_HEADER)
        if (data[i] == 0x01 and data[i+1] == 0x00 and 
            data[i+2] == 0x00 and data[i+3] == 0x00):
            rec_size = struct.unpack_from('<I', data, i+4)[0]
            # Valid EMF header size is between 88 and 200 bytes
            if 88 <= rec_size <= 200:
                # Try to find EMF end (record type 14 = EMR_EOF)
                end = -1
...
def parse_questions(xml_text):
    ...
    for i, block in enumerate(blocks):
        ...
        if not q_text and not has_image:
            continue
...
def parse_exe():
    try:
        ...
        if emf_images:
            # Convert all EMF to PNG
            png_images = []
            for offset, emf_bytes in emf_images:
                b64 = emf_to_png(emf_bytes)
                if b64:
                    png_images.append(b64)
            
            # Match images to questions that need them
            img_idx = 0
            for q in questions:
            ...
            # If no hasImage flags but images exist, assign sequentially
