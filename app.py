    ...
    try:
        ...
        r = subprocess.run(
            ['libreoffice', '--headless', '--norestore',
             '--convert-to', 'png', '--outdir', '/tmp', emf_path],
        ...
        print(f"EMF error: {e}", file=sys.stderr)
        try:
            if os.path.exists(emf_path): os.unlink(emf_path)
        except: pass
        return None
...
def parse_questions(xml_text):
    ...
    for i, block in enumerate(blocks):
        ...
        # Rasmli savolni XML dan aniqlaymiz
        has_image = bool(re.search(r'<ImageData>|<HasImage>\s*true|<Picture>', block, re.IGNORECASE))
...
def parse_exe():
    try:
        ...
        print(f"Questions: {len(questions)}, hasImage: {sum(1 for q in questions if q['hasImage'])}", file=sys.stderr)
        if not questions:
            return jsonify({'error': 'Savollar topilmadi'}), 400

        # Faqat rasmli savollar sonini hisoblaymiz
        img_questions = [q for q in questions if q['hasImage']]
        print(f"Image questions: {len(img_questions)}", file=sys.stderr)
        ...
        if img_questions:
            ...
            if emf_images:
                # Faqat kerakli miqdorda rasmlarni o'giramiz
                needed = len(img_questions)
                png_images = []
                for emf_bytes in emf_images[:needed]:
                    b64 = emf_to_png(emf_bytes)
                    if b64:
                        png_images.append(b64)
                
                print(f"PNG converted: {len(png_images)}/{needed}", file=sys.stderr)
                
                img_idx = 0
                for q in questions:
                    if q['hasImage'] and img_idx < len(png_images):
                        q['img'] = png_images[img_idx]
                        img_idx += 1
        else:
            # hasImage belgisi yo'q — barcha EMF larni o'giramiz
            emf_images = find_emf_images(data)
            print(f"No hasImage flags, EMF found: {len(emf_images)}", file=sys.stderr)
            if emf_images:
                png_images = []
                for emf_bytes in emf_images:
                    b64 = emf_to_png(emf_bytes)
                    if b64: png_images.append(b64)
                print(f"PNG converted: {len(png_images)}", file=sys.stderr)
                img_idx = 0
                for q in questions:
